...
final class EditingShipFittingViewModel: ShipFittingViewModelProtocol {
    ...
    private func loadShip(with shipID: UUID) {
        DispatchQueue.global(qos: .background).async {
            let ship = ShipManagementService.shared.loadShipForEditing(with: shipID)
            self.ship.send(ship)
        }
    }
    ...
    private func handleOkButtonTapped() {
        ...
        ShipManagementService.shared.saveEditedShip(ship: ship)
    }
